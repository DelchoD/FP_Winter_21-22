...
type Vector a = (Int, Int, Int)

crossProduct :: Vector Int -> Vector Int -> Vector Int
crossProduct (x1, y1, z1) (x2, y2, z2) = (y1*z2-z1*y2, z1*x2-x1*z2, x1*y2-y1*x2 )

magnitude :: Vector Int -> Double
magnitude (x, y, z) = sqrt(fromIntegral x* fromIntegral x+ fromIntegral y* fromIntegral y+fromIntegral z* fromIntegral z)